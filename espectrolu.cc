...
#define N 4096				       // Número máximo de Datos por Canal
using namespace std;
...
if (argc<2){
	...
	switch (argc)
		{
		case 2: m=sprintf(arch,"%s.spe",argv[1]);
			break;
		case 3: m=sprintf(arch,"%s.spe",argv[1]);
			reb=atoi(argv[2]);
			break;
		case 4: m=sprintf(arch,"%s.spe",argv[1]);
			fin=atoi(argv[2]);ffi=atoi(argv[3]);
			a=fin;b=ffi;
			break;
			
		case 5: {
			m=sprintf(arch,"%s.spe",argv[1]);
			reb=atoi(argv[2]);
...
if(!*in) 
	...
	for (i=0;i<819;i++)
		{*in >> dx  >> dy1 >> dy2 >> dy3 >> dy4 >> dy5;                  
		x[i*5+1]=i*5+1;x[i*5+2]=i*5+2;x[i*5+3]=i*5+3;x[i*5+4]=i*5+4;x[i*5+5]=i*5+5;
		y[i*5+1]=dy1;y[i*5+2]=dy2;y[i*5+3]=dy3;y[i*5+4]=dy4;y[i*5+5]=dy5;
		}
