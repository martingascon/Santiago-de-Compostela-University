...
TH1F *hr = c1->DrawFrame(0,0,100,30000);      // draw a frame to define the range
...
m=sprintf(ar,"y(x) = %.1f+%.1f*exp(x*%.4f)+%.1f*exp(-x*%.4f))",d,e,f,g,h);//,d,e);
...
legend->AddEntry(gr1," CsI(Tl) 10 cm + S8664","p");
legend->AddEntry(gr2," CsI(Tl) 20 cm + Pyrex","p");
