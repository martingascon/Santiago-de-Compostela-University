...
TH1I *hist = new TH1I("hist max-min",arch,400,-200,200); 		// para RPCs
TH1I *hist2 = new TH1I("hist max",arch,200,-100,100);		 // para RPCs
TH1I *hist3 = new TH1I("hist min",arch,200,-100,100);		 // para RPCs
TH1I *hist4 = new TH1I("min value",arch,100,-0.25,0);	
/*
TH1I *J1 = new TH1I("J1",arch,2500,1,2500); 
TH1I *J2 = new TH1I("J2",arch,2500,1,2500); 

TFile *fichroot=new TFile("../analisis/pulsos.root","UPDATE");	// abre un fichero root
TTree *tree = new TTree("tree","signals");			// crea un Tree
tree->Branch("signals_ch1","TH1I",&H1,32000,0);			// crea una rama del tree
tree->Branch("signals_ch2","TH1I",&H2,32000,0);			// crea una rama del tree
tree->Branch("filters_ch1","TH1I",&J1,32000,0);			// crea una rama del tree
tree->Branch("filters_ch2","TH1I",&J2,32000,0);			// crea una rama del tree
tree->Branch("maximum_ch1",&Emax1,"Emax1/F");
tree->Branch("maximum_ch2",&Emax2,"Emax2/F");
tree->Branch("tmax1_ch1",&k1,"k1/F");
tree->Branch("tmax2_ch2",&k2,"k2/F");
...
{	m=sprintf(arch,"%d.txt",j);  				//cargo en arch el nombre del fichero
	...
	else    
	{
		...
		/////////////////////////////////////////////////////////////////////////////////// para RPCs
		Float_t Amin1; Int_t kmin;
		Emax1=0;Emax2=0; Emin1=0; Emin2=0;k1=0;k2=0;k3=0;k4=0,kmin=0;
		Amin1=0;	
		for (k=200;k<1500;k++)				// para RPCs    
             	  	{
		 	if (Amin1<-y1[k])   
				{
				Amin1=-y1[k];  	//calculo del mínimo
				kmin=k;				
				}			
			if (Emax1<phi1[k])  
				{
			...
			if (Emax2<phi2[k])  
				{		
				Emax2=phi2[k];  		// calculo el máximo para 2
				k2=k;
				}

		   	if (Emin1<-phi1[k])   			
				{				
				Emin1=-phi1[k];          	// calculo el mínimo para 1
				k3=k; 				
				} 				
		   	if (Emin2<-phi2[k])  			
				{			 	
				Emin2=-phi2[k];  	 	// calculo el mínimo para 2
				k4=k;				
				}
			}


		//cout <<  Emax1 << " " << Amin1 <<  " " << kmin << " " << k1 << " " << k2 << " " << k3 << " " << k4  << endl; 	// para RPCs 
		k5=(k1+k3)/2; 					// promedio de máximo y minimo para 1
		k6=(k4+k2)/2; 					// promedio de maximo y minimo para 2		
		//if (k5<0) k5 = -k5;
		//if (k6<0) k6 = -k6;
		k7=k5-k6;	
		//if (k7<0) k7 = -k7;			// diferencia de los promedios pulso 2 - pulso 1

		//if ((j%100)==0) cout << k7 << " " << k1-k2 << " " << k3-k4 << endl;
		//if (kmin>675&&kmin<695&&Amin1<0.035)

		//	{
			hist->Fill(k7);  	     			// histo de las diferencias entre promedios			
			hist2->Fill(k1-k2);  				// histo de las diferencias entre máximos	
			hist3->Fill(k3-k4);  				// histo de las diferencias entre mínimos
			hist4->Fill(-Amin1);  				// histo de las amplitudes máximas del pulso (es negativo)
			*out4 <<  k1-k2 << " " << Amin1  << endl;   
		//	}
//////////////////////////////////////////////////////////////////////////////////////////////////////// para RPCs
		//tree->Fill();
		//tree->Write();
