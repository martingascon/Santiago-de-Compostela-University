...
gr1->GetXaxis()->SetTitle("tiempo (minutos)");			// Escribe como titulo del eje x lo que hay en ejex
gr1->GetYaxis()->SetTitle("Resolucion (%)");	    	// Escribe como titulo del eje y lo que hay en ejey
...
TF1 *f1 = new TF1("f1","([0]+[1]*exp(-x*[2]))",1,80); 		//*([5]-[3]*exp(-x*[4]))",0,14); 
f1->SetParameter(0,6.2);					// Defino una función TF1 llamada f1 con parametros
f1->SetParameter(1,0.4);						// 0 y 14 son el intervalo sobre el que se define f1
f1->SetParameter(2,0.2);						// Asigno valores a los parametros (numero, valor)
...
TLegend *legend=new TLegend(0.64,0.71,0.99,0.76,"funcion ajustada");	// Declaración de la leyenda
legend->SetTextFont(72);					// Tipo de fuente de la leyenda
legend->SetTextSize(0.03);					// Tamaño de la fuente de la leyenda
m=sprintf(ar,"y(x) = (%.1f+%.1f*exp(-x*%.1f))",f1->GetParameter(0),f1->GetParameter(1),f1->GetParameter(2));  									// Introduce una leyenda con datos
