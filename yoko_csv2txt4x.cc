...
char arch[100],arch2[100],arch3[100]="../pulsos/tiempos.txt",ci[100]="", *dat0, *dat1, *dat2, *dat3, *dat4, *dat5;
		...
		*out << " <<START>> " << endl;		
		...
		for (i=0;i<(N-1);i++)                                        // cargo los datos en datosy, y2
			...
			*in >> ci; 
			...
			x[i]=atof(dat1); y[i]=atof(dat2); y2[i]=atof(dat3);y3[i]=atof(dat4);
			*out << x[i] << " " << y[i] << " " << y2[i] << " " << y3[i] <<  endl;
			}   
		*out << " <<END>> " << endl;	               
		delete out;
