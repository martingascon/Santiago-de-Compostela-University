...
Float_t datosx[N],datosy[N],datosy2[N],errorx[N],errory[N],errory2[N],d,e,h,f,pra,prb; 		      
Int_t i,k,m,m1;  
char fich[100],serie1[100]="serie 1",serie2[100]="serie 2";
char fitf[100]="(1+[0]*((x-6.5)/6.5))",ar[100],ar2[100];// nr
//char fitf[100]="1+[0]*((x-10)/10)",ar[100],ar2[100];//)" 
gROOT->SetStyle("Bold"); 
...
TH1F *hr = c1->DrawFrame(0,0,20,2);      
hr->SetXTitle("Distance from the detector (cm)");	// Escribe como titulo del eje x lo que hay en ejex
hr->SetYTitle("Normalized Light Output");   	        // Escribe como titulo del eje y lo que hay en ejey
...
for (k=0;k<i-1;k++)
	...
	y2[k]=datosy2[k]/prb;
	ey[k]=errory[k]/1000;
	ey2[k]=errory2[k]/1000;
	}

//fitf[100]="1+[0]*((x-k/2)/(k/2))"; 
...
TLegend *legend=new TLegend(0.54,0.81,0.79,0.86);	// Declaración de la leyenda
TLegend *legend2=new TLegend(0.59,0.2,0.87,0.26);
