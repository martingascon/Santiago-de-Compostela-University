...
{	m=sprintf(arch,"%d.txt",j);  				//cargo en arch el nombre del fichero
	...
	else    
	{
		...
		//suple= sqrt((integ1[j]*1918)*(integ1[j]*1918) + (integ2[j]*57.2)*(integ2[j]*57.2))/1.4142;
/*		Emax2=phi2[0];
		for (k=0;k<(I-F);k++)
...
for (j=nini;j<(nfin+1);j++)
	...
	H1->Fill((integ1[j])*1918);             // lleno el histograma
	H2->Fill((integ2[j])*57.2);             // lleno el histograma
	H3->Fill((integ3[j]));             // lleno el histograma
