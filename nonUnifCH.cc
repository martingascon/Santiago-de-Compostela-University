...
Float_t max_a,max_b,min_a,min_b,g1,g2; 	
...
for (k=0;k<i-1;k++)
	...
	y[k]=datosy[k];
	y2[k]=datosy2[k];
	if (datosy[k]>max_a) max_a=datosy[k];if (datosy2[k]>max_b) max_b=datosy2[k];
	if (datosy[k]<min_a) min_a=datosy[k];if (datosy2[k]<min_b) min_b=datosy2[k];
	ex[k]=errorx[k];         
	ey[k]=errory[k];
	ey2[k]=errory2[k];
	pra=pra+datosy[k];
