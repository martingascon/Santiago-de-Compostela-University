...
char ch, titulo[100]="Spectrum",arch[100],ci[100]="",ejex[100]="time (channels)",ejey[100]="voltaje (V)",tituloa[100]="Yokogawa 9140DL (ch1)",titulob[100]="Yokogawa 9140DL (ch2) ",tituloc[100]="XFIR de ch1";
...
for (i=0;i<N;i++)    {phi1[i]=0,phi2[i]=0,y1[i]=0;}       

TGraph *gr3 = new TGraph(N,x,y1);			// Declaración del gráfico.
...
TGraph *gr1 = new TGraph(I-F,x,phi1);			// Declaración del gráfico.
...
gr1->SetTitle(tituloc);					// Escribe como título del grafico lo que hay en titulo
...
{	m=sprintf(arch,"%d.txt",j);  				//cargo en arch el nombre del fichero
	...
	else    
	{
		...
		while (i!=0);	
			...
			{
			*in  >> datosy2[i] >> datosy[i];                 // desde -40 a 40 us (otra escala)
...
/*		c1->cd(1);
		...
		gr3->SetLineColor(2);					// Color de la linea 0=blanco 1=negro, 2=rojo
		gr3->SetLineWidth(1);					// Ancho de la linea
		gr3->SetMarkerColor(2);					// Color del punto 0=blanco 1=negro, 2=rojo
		gr3->SetMarkerStyle(19);				// Estilo del punto 21 = cuadrado
		gr3->SetTitle("Shaper pulse");					// Escribe como título del grafico lo que hay en titulo
		gr3->GetXaxis()->SetTitle(ejex);			// Escribe como titulo del eje x lo que hay en ejex
		gr3->GetYaxis()->SetTitle("Shaper amplitude (a.u.)");			// Escribe como titulo del eje y lo que hay en ejey
		gr3->GetXaxis()->CenterTitle();				// Para centrar el titulo del eje x
...
/*		c1->cd(2);
		TGraph *gr1 = new TGraph(I-F,x,phi2); //cambiado phi1 por phi2
		gr1->SetLineColor(2);					// Color de la linea 0=blanco 1=negro, 2=rojo
		gr1->SetLineWidth(1);					// Ancho de la linea
		gr1->SetMarkerColor(2);					// Color del punto 0=blanco 1=negro, 2=rojo
		gr1->SetMarkerStyle(19);				// Estilo del punto 21 = cuadrado
		gr1->SetTitle("FIR pulse");					// Escribe como título del grafico lo que hay en titulo
		gr1->GetXaxis()->SetTitle(ejex);			// Escribe como titulo del eje x lo que hay en ejex
		gr1->GetYaxis()->SetTitle("FIR amplitude (a.u.)");			// Escribe como titulo del eje y lo que hay en ejey
		gr1->GetXaxis()->CenterTitle();				// Para centrar el titulo del eje x
		...
		Emax1=datosy[0];            // esto esta agregado
		for (k=0;k<(I-F);k++)
             	   if (Emax1<datosy[k]) 
			{Emax1=datosy[k];           		 	// calculo el máximo de la energía
			can=k;		  	
			}
